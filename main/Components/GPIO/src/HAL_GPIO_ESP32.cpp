...
bool HAL_GPIO_ESP32::setDirection(const HAL_BASE<gpio_num_t>::io_def_t& dir)
{
    ...
    #ifdef __ESP32__
    // set the direction
    if(!gpio_set_direction(getPin(), dir == HAL_BASE<gpio_num_t>::io_def_t::OUTPUT ? GPIO_MODE_OUTPUT
...
bool HAL_GPIO_ESP32::setValue(const HAL_BASE<gpio_num_t>::io_val_t& val)
{
    ...
    #endif
    #ifdef __ESP32__
    if(gpio_set_level(getPin(), val == HAL_BASE<gpio_num_t>::io_val_t::HIGH ? true : false) == ESP_OK)
    {
        return true;
    }
...
HAL_BASE<gpio_num_t>::io_val_t HAL_GPIO_ESP32::getValue() const
{
    ...
    #endif
    #ifdef __ESP32__
    return gpio_get_level(getPin()) == 1 ? HAL_BASE<gpio_num_t>::io_val_t::HIGH : HAL_BASE<gpio_num_t>::io_val_t::LOW;
    #endif
    return HAL_BASE<gpio_num_t>::io_val_t::LOW;
}

bool HAL_GPIO_ESP32::installInterruptDriver(void (*callbackfunction)(void*),
...
bool HAL_GPIO_ESP32::enableInterrupt()
{
    ...
    #endif
    #ifdef __ESP32__
    return gpio_intr_enable(getPin()) == ESP_OK ? true : false; // Enable the pin for interrupts
    #endif
    return false;
}
...
bool HAL_GPIO_ESP32::disableInterrupt()
{
    ...
    #endif
    #ifdef __ESP32__
    return gpio_intr_disable(getPin()) == ESP_OK ? true : false; // disable the pin for interrupts
    #endif
    return false;
}
