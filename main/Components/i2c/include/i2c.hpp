...
class i2c {
 ...
 public:
  ...
  template <size_t N>
  bool writeMultiBytes(const uint8_t& slave_address,
                       const std::array<uint8_t, N>& array) {
    return m_hal.writeMultiBytes(slave_address, array.data(), array.size());
  }

  template <size_t N>
  bool readMultiBytes(const uint8_t& slave_address,
...
class friend_i2c {
 public:
  ...
  ~friend_i2c(){};
  auto& getHAL() { return m_sensor->m_hal; }
  auto word2bytes(uint16_t data) { return m_sensor->word2bytes(data); }
