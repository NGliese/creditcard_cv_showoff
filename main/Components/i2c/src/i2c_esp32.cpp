...
bool i2c_esp32::slavePresent(uint8_t address) {
  ...
  esp_err_t espRc =
      ::i2c_master_cmd_begin(m_portNum, cmd, 100 / portTICK_PERIOD_MS);
  i2c_cmd_link_delete(cmd);
  return espRc ==
         0;  // Return true if the slave is present and false otherwise.
#else
  return true;
#endif
}

bool i2c_esp32::writeByte(const uint8_t& slave_address, const uint8_t& byte) {
#ifdef DEBUG
...
bool i2c_esp32::writeMultiBytes(const uint8_t& slave_address,
                                const uint8_t* data, const size_t& length) {
#ifdef DEBUG
  std::cout << " >> i2c_esp32::writeMultiBytes >> \n";
#endif
  // do magic
  // Executable code:
  auto err = initialize_transaction();
  ...
  insert_start_bit();
  insert_write_bytes(slave_address, data, length);
  insert_stop_bit();
...
uint8_t i2c_esp32::readByte(const uint8_t& slave_address) {
#ifdef DEBUG
...
bool i2c_esp32::readMultiBytes(const uint8_t& slave_address, uint8_t* data,
                               const size_t& length) {
  auto err = initialize_transaction();
  if (err != true) {
    return err;
  ...
  insert_start_bit();
  insert_read_bytes(slave_address, data, length);
  insert_stop_bit();
...
bool i2c_esp32::readMultiBytes(const uint8_t& slave_address,
...
bool i2c_esp32::initialize_transaction() {
#ifdef DEBUG
...
bool i2c_esp32::insert_start_bit() {
#ifdef DEBUG
...
bool i2c_esp32::insert_write_byte(const uint8_t& slave_address,
                                  const uint8_t& byte) {
#ifdef DEBUG
...
#ifdef __ESP32__
  esp_err_t errRc = ::i2c_master_write_byte(
      m_cmd, (slave_address << 1) | I2C_MASTER_WRITE, false);
  if (errRc != ESP_OK) {
    return false;
  }
...
bool i2c_esp32::insert_write_bytes(const uint8_t& slave_address,
                                   const uint8_t* bytes, const size_t& length) {
#ifdef DEBUG
  LOG_PRINT_INFO(LOG_TAG, ">> i2c_esp32::insert_write_bytes >> ");
#endif
  // Executable code:
  ...
  errRc = ::i2c_master_write(m_cmd, bytes, length, false);
  if (errRc != ESP_OK) {
    return false;
  }
#endif
...
bool i2c_esp32::insert_read_byte(const uint8_t& slave_address, uint8_t* byte) {
#ifdef DEBUG
  ...
  esp_err_t errRc = ::i2c_master_write_byte(
      m_cmd, (slave_address << 1) | I2C_MASTER_READ, false);
  if (errRc != ESP_OK) {
    return false;
  }
...
bool i2c_esp32::insert_read_bytes(const uint8_t& slave_address, uint8_t* bytes,
                                  const size_t& length) {
#ifdef DEBUG
  LOG_PRINT_INFO(LOG_TAG, ">> i2c_esp32::insert_read_bytes >> ");
#endif
  // Executable code:
#ifdef __ESP32__
  ...
  ESP_ERROR_CHECK(::i2c_master_read(m_cmd, bytes, length, I2C_MASTER_ACK));
#endif
...
bool i2c_esp32::insert_stop_bit() {
#ifdef DEBUG
...
bool i2c_esp32::execute_transaction() {
#ifdef DEBUG
...
#ifdef __ESP32__
  esp_err_t errRc =
      ::i2c_master_cmd_begin(m_portNum, m_cmd, 1000 / portTICK_PERIOD_MS);
