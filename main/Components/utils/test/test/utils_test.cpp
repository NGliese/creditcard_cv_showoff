...
// test init
TEST(Utils_GRP, leastsegbyte)
{
	uint16_t test = 0x0292;
	LONGS_EQUAL(0x92, basic_utils::getLeastSignificantByte(test));
}
// test init
TEST(Utils_GRP, mostsegbyte)
{
	uint16_t test = 0x0292;
	LONGS_EQUAL(0x02, basic_utils::getMostSignificantByte(test));
}

// test init
TEST(Utils_GRP, shit8216)
{
	uint8_t b1 = 0x10;
	uint8_t b2 = 0x20;
	LONGS_EQUAL(0x1020, basic_utils::shift8BitTo16(b1, b2));
}