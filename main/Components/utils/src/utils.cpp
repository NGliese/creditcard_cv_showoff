...
uint8_t basic_utils::getMostSignificantByte(uint16_t data)
{
	return static_cast<uint8_t>(data >> 8);
}

uint16_t basic_utils::shift8BitTo16(const uint8_t& b1, const uint8_t& b2)
{
	return (b1 << 8 | b2);
}
...
bool basic_utils::shift32BitTo8(const uint32_t& b32, uint8_t& b1, uint8_t& b2, uint8_t& b3,
										 uint8_t& b4)
{
#ifdef DEBUG
